...
<div class="max-w-2xl">
        ...
        <form method="POST" action="{{ route('admin.categories.update', $category) }}" class="space-y-6">
            ...
            <div>
                ...
                <div class="space-y-3 mb-4">
                    <label class="flex items-center p-3 border border-gray-600 rounded-lg cursor-pointer hover:bg-gray-800/50">
                        <input type="radio" name="category_type" value="main" {{ !old('parent_id', $category->parent_id) ? 'checked' : '' }}
                        ...
                        <div class="ml-3">
                            ...
                            <span class="block text-gray-400 text-sm">This is a top-level category</span>
                        </div>
                    </label>
                    
                    <label class="flex items-center p-3 border border-gray-600 rounded-lg cursor-pointer hover:bg-gray-800/50">
                        <input type="radio" name="category_type" value="sub" {{ old('parent_id', $category->parent_id) ? 'checked' : '' }}
                               class="w-4 h-4 text-blue-600 bg-gray-800 border-gray-600 focus:ring-blue-500 focus:ring-2"
                               onchange="toggleParentSelect(true)">
                        <div class="ml-3">
                            <span class="block text-white font-medium">Subcategory</span>
                            <span class="block text-gray-400 text-sm">This belongs under another category</span>
                        </div>
                    </label>
                </div>
                
                <!-- Parent selection -->
                <div id="parentSelectContainer" style="{{ old('parent_id', $category->parent_id) ? 'display: block;' : 'display: none;' }}">
                    <label for="parent_id" class="block text-sm font-medium text-gray-300 mb-2">
                ...
                <p class="text-gray-500 text-sm mt-1">
                    @if($category->children->count() > 0)
                        <span class="text-yellow-400">⚠️ This category has subcategories and cannot become a subcategory itself.</span>
                    @else
                        Choose whether this should be a main category or subcategory.
                    @endif
                </p>
                @error('parent_id')
            ...
            <div>
                <label for="name" class="block text-sm font-medium text-gray-300 mb-2">
                    Category Name (English) <span class="text-red-400">*</span>
                </label>
                <input type="text" 
                       id="name" 
                       name="name" 
    ...
    <div class="mt-6 admin-card rounded-lg p-6">
        <h3 class="text-lg font-semibold text-white mb-4">Category Statistics</h3>
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div class="text-center">
                <p class="text-2xl font-bold text-blue-400">{{ $category->articles()->count() }}</p>
                <p class="text-gray-400 text-sm">Total Articles</p>
            </div>
            <div class="text-center">
                <p class="text-2xl font-bold text-green-400">{{ $category->publishedArticles()->count() }}</p>
                <p class="text-gray-400 text-sm">Published Articles</p>
            </div>
            <div class="text-center">
                <p class="text-2xl font-bold text-orange-400">{{ $category->articles()->where('is_published', false)->count() }}</p>
                <p class="text-gray-400 text-sm">Draft Articles</p>
            </div>
        </div>
    </div>
</div>
...
@push('scripts')
<script>
    // Auto-generate slug from name if slug is empty (only for manual changes)
    ...
    function toggleParentSelect(isSubcategory) {
        ...
        if (isSubcategory) {
            ...
            parentSelect.value = ''; // Clear selection when hiding
        }
    }
</script>
@endpush
